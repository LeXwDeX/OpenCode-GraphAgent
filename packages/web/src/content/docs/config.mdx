...
Use the root-level `question_timeout` option to choose how many seconds OpenCode waits for an answer to an interactive question. The default is 60 seconds.
...
The value must be a positive integer. Selecting an option or focusing the custom answer input starts a response phase. Each interaction refreshes an inactivity timer of `question_timeout` seconds, with a five-minute maximum response phase. Moving between options or question tabs alone does not start that phase. If either timer expires, OpenCode tells the agent that you are temporarily away. It offers the first option marked "(Recommended)" as a fallback candidate, or the first option if none is marked. Even when several options are marked or multiple selections are allowed, it offers only one candidate. The agent may use that choice only when already authorized and safe. Silence never authorizes new scope or irreversible action. If no safe choice exists, the agent defers that step, continues independent authorized work, and reports the blocker. Dismissing a question also defers its dependent step and tells the agent not to ask the same question again in that turn.
