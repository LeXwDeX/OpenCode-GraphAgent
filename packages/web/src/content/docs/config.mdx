...
The value must be a positive integer. Selecting an option or focusing the custom answer input cancels the timeout for the entire request. Moving between options or question tabs does not cancel it. If the timeout expires, OpenCode tells the agent that you are temporarily away so it can analyze the choices and continue within the task's existing authorization.
