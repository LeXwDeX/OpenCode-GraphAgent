...
- `FINAL-ACCEPTANCE.md`：最终 H0/P0、候选制品与真实 Qwen 配对验收的脱敏结论、hash 和已知限制。
...
- `scripts/performance-gate.ts` 与 `performance-result.example.json`：冻结门槛的结果判分器和输入模板；正式增量采样结果见
  `FINAL-ACCEPTANCE.md`。
- `fixtures/performance-gate-*.synthetic.json`：仅验证预算前性能门禁的正向输入与 fail-closed 绕过拒绝，不是实际性能证据。
- `H0-P0-HARNESS-PLAN.md`：零模型 host 触发与性能 runner 的实现/复验方案；历史计划保留供追溯，最终结果见
  `FINAL-ACCEPTANCE.md`。
...
原始证据仍在本机临时目录中，不属于可提交工件。H0 完整 host 日志含 session ID、测试 provider 身份和临时路径；outbound 含 fixture 正文。真实模型证据还包含 prompt、回答、工具正文和 before/after history。它们只以逻辑名称、白名单聚合值和 SHA-256 出现在公开摘要中。临时目录可能被系统清理，因此公开摘要只用于交接，不替代原始证据的独立复核。
