...
- 日期：2026-09-16
- 状态：S01 接入契约已按首轮审计修订，等待父任务复审；产品实现仍待开始，本文不代表功能已经上线或通过运行验收。
...
宿主还必须维护一个有界的“工具执行来源身份台账”。台账随 location/session 与 instance 生命周期存在，只记录成功结算调用的 `{sessionID, assistantMessageID, callID, toolName, sourceKind, registrationGeneration}`；不得记录输入、输出正文、路径、指纹、候选或折叠计划，不持久化，也不得跨 session 查询。session 删除、instance 关闭或 registration generation 变化时清除；重启前历史因没有可信来源记录而保护。该台账只回答“这个已结算调用由哪个注册项执行”，不是跨请求重复内容缓存；正文比较、分组、预算和计划仍在每次请求中从当前输入独立计算。

OpenCode 的 provider 适配必须复用宿主现有调用 ID 变换。实施时把 `ProviderTransform.message` 当前 Claude/Mistral 使用的 ID scrub 提取为唯一共享函数，由现有消息变换和折叠 wire 映射共同调用，不另写第二份规则。共享规划器只产出稳定 source/witness 引用，不包含 provider 特例；适配器在全部 provider 消息变换完成后，用同一函数得到最终可见 ID，并校验 call/result 唯一配对、正文位置和无碰撞，再用最终 witness ID 生成占位并原子投影。未改写 ID 或已由该共享函数证明的非碰撞映射都是正向路径；只有碰撞、未知变换或最终结构不唯一时跳过。

Core 内置 `read` 的首个正向映射限定为已解码且完整符合 `ReadTool.TextPage` 的结果。候选正文严格取 `TextPage.content`；同组 source/witness 的完整输入，以及 `type`、`mime`、`offset`、`truncated`、`next` 等全部外层字段必须逐字段相等。投影只在本次请求的独立 wire 副本中替换 JSON 结果的 `content` 值，保留 JSON result 类型、全部外层字段和 provider 协议结构；持久化原记录及工具输出 schema 均不修改。若适配器不能在最终 wire 中唯一定位并往返验证该字段，或遇到 `ListPage`、媒体、二进制及未知 read 结果，则保护该项。

优先沿用现有 Effect 作用域；无新增独立后台服务。身份台账按项目 `AppLayer` / `InstanceState` 约定集成，不得扩展成保存内容的全局可变缓存。
...
核心实际输入至少包含：请求用途、已解析开关、模型预算、完整步骤边界、每个候选的身份/顺序/工具类型/完整参数/正文/资格信息、经台账证明的来源身份和最终请求对应关系。`ToolSourceIdentity` 只由宿主适配层查表后作为资格证明输入；纯规划器不访问、更新或缓存台账。

映射缺失或一对多语义不明确时保护该项；如果因此不能可靠估算或确认整个计划的身份，跳过整次投影。以数组位置作为辅助定位可以，但不能只靠位置判断身份。

转换链可能拆分工具结果、抽取附件或携带 provider 元数据。首版只支持适配器明确验证的正文槽位：OpenCode 的纯文本结果，以及 Core `ReadTool.TextPage.content` 的已知 JSON 槽位。provider 执行工具、其他未知结构化结果或附带无法证明安全的元数据均跳过。
...
重复组依据为：已核实工具种类、完整输入参数、完整输出正文；Core `ReadTool.TextPage` 还必须包含除 `content` 外的完整外层字段。
...
每次请求独立计算，不留跨请求或跨会话的内容、指纹、候选、预算或计划缓存。规范化与指纹扫描预计随输入总字节数增长；以字节量计性能，不能只以消息条数计。

唯一允许跨普通请求保留的是第 4/5 节定义的有界工具来源身份台账：它只保留调用与注册来源身份，不参与正文相等、候选选择或计划复用，并随 session/instance/generation 清除。重启前调用因台账不存在而跳过，不能用持久化历史中的工具名补推来源。
...
| U23  | 来源台账存在/缺失/代际变化                  | 仅可信同代内置来源可入选                |
| U24  | 重启前历史、跨 session 同名调用             | 不补推来源且不跨 session 复用           |
...
| I13  | Core `ReadTool.TextPage` 重复结果                        | 只替换 wire `content`，外层字段/输入/存储/schema 不变 |
| I14  | Core list/media/binary/未知 read 结果                    | 保守跳过；不能用 grep/glob 测试替代                   |
| I15  | Claude/Mistral 非碰撞 ID 变换                            | 复用宿主共享函数并用最终 ID 正向折叠                  |
| I16  | ID 变换碰撞或未知 transformer                            | 不部分投影，原请求保持不变                            |
...
实施已按项目 SpecGit 2 流程查重并建立 Why/Scope/Approach/Acceptance 完整 Issue #605，S01 文档位于 Draft PR #606；进入每个代码步骤前仍须读取实际分支的 package-local `AGENTS.md`。该事实不表示 S01 已通过父任务复审，也不授权 ready、merge、发布或启动 S02。
